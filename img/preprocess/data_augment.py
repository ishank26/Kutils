...
def rotate(img_data):
    ...
    for i in range(len(img_data)):
        ...
        rot=img.rotate_ccw_about_centre(rotation[j], degrees=True, retain_shape=True)
        rotated.append(rot)
    return rotated
...
def out_img(list1,list2,type):
    for j in range(len(list2)):
        mo.export_image(list2[j], "rot/rot_"+str(type)+str(j)+".jpg",overwrite=True)

    for i in range(len(list1)):
        mo.export_image(list1[i], "trans/trans_"+str(type)+str(i)+".jpg",overwrite=True)
